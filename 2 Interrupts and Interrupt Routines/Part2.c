...
int main(void)
{
    ...
    // check @TODO You need to add in the configuration for the Green LED
    ...
    __bis_SR_register(GIE);                 // Enter LPM3 w/interrupt MUST BE THERE to interpret interrupts
    ...
    while(1)
    {
        ...
        { P1OUT ^= BIT0;                  // P1.0 = toggle
           P6OUT = ~BIT6;}                // P6.6 is off
        else
        {  P1OUT = ~BIT0;                 // Set P1.0 to 0
           P6OUT ^= BIT6;}                // P6.6 = toggle but in an if statement way
...
__interrupt void Port_2(void) //little as possible code during an interrupt
{
